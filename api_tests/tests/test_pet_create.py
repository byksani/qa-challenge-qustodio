...
class TestCreatePet:
    def test_create_new_pet_valid_payload_success_result(self, pet_methods, unique_user_data):
        status_code, response_context = pet_methods.create_pet(unique_user_data)

        assert status_code == 200, \
            f'Expected status 200, but got {status_code}.'

        assert response_context == unique_user_data, \
            'Response body does not match the request payload.'

    def test_create_new_pet_empty_payload_error_405(self, pet_methods):
        status_code, response_context = pet_methods.create_pet(None)
    ...
    def test_create_pet_with_empty_body_returns_valid_structure(self, pet_methods):
        status_code, response_context = pet_methods.create_pet({})
    ...
    def test_create_pet_with_text_plain_content_type_returns_415(self):
        payload = '{"id": 123}'
        headers = {"Content-Type": "text/plain"}

        response = requests.post(Urls.BASE_URL, data=payload, headers=headers)
